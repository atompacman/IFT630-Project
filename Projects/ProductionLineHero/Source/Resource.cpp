...
Dollar BasicResource::worth() const
{
    return BASIC_RSRC_BASE_VALUE * pow(BASIC_RSRC_RAFFINEMENT_VALUATION, min(m_RaffinementLvl, 3));
}
...
Dollar CompositeResource::worth() const
{
    double sum = m_SubResource[0]->worth();
    sum += m_SubResource[1]->worth();
    if (m_SubResource[2])
    {
        sum += m_SubResource[2]->worth();
    }
    return pow(sum, pow(CMPST_RSRC_RAFFINEMENT_VALUATION, 1 + m_RaffinementLvl));
}